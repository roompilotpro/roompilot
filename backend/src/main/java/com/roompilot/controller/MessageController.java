...
public class MessageController {
    ...
    @Operation(summary = "Create a new message", description = "Creates a new message with the provided content")
    @ApiResponse(responseCode = "200", description = "Message created successfully")
    @PostMapping
    public Message createMessage(
            @Parameter(description = "Message content in JSON format with 'content' field")
            @RequestBody Map<String, String> request) {
        String content = request.get("content");
        return messageService.createMessage(content);
    }
    
    @Operation(summary = "Update a message", description = "Updates an existing message with new content")
    @ApiResponses(value = {
