 ...
 * Handles OAuth callbacks, login, role selection, and user profile access.
 */
@RestController
...
public class AuthController {
    ...
    /**
     * Select role during user onboarding.
     * Users must select HOST or RESIDENT role after first login.
     *
     ...
     */
    @PostMapping("/role")
    @Operation(summary = "Select user role", description = "Set role to HOST or RESIDENT during onboarding")
    ...
    /**
     * Logout endpoint.
     * Currently just returns success - token invalidation happens client-side.
    ...
    @Operation(summary = "Development login", description = "For testing only - requires ENABLE_DEV_AUTH=true")
    public ResponseEntity<AuthResponse> devLogin(@RequestBody Map<String, String> request) {
        try {
            ...
            if (request.containsKey("role") && request.get("role") != null) {
                role = UserRole.valueOf(request.get("role").toUpperCase());
            }
            ...
            logger.error("Dev authentication failed: {}", e.getMessage());
